...
// src/index.test.ts
const index_1 = require("./index"); // Adjust the import path if your file is not in src/index.ts
describe("textToHtml", () => {
    ...
    test("should convert text to H1 with black text and red background", () => {
        ...
        const expected = '<h1><span style="color: #000000; background-color: #ff0000;">Hello World</span></h1>';
        expect((0, index_1.textToHtml)(message, stylesFormat)).toBe(expected);
    });
    ...
    test("should default to a P tag when no stylesFormat is provided", () => {
        ...
        expect((0, index_1.textToHtml)(message)).toBe(expected);
    });
    ...
    test("should default to a P tag and apply styles when htmlTag is omitted", () => {
        ...
        const expected = '<p><span style="color: #0000ff; font-size: 16px;">Styled paragraph</span></p>';
        expect((0, index_1.textToHtml)(message, stylesFormat)).toBe(expected);
    });
    ...
    test("should correctly convert camelCase CSS properties to kebab-case", () => {
        ...
        const expected = '<div><span style="font-size: 20px; line-height: 1.5; text-decoration: underline; border-bottom: 1px solid green;">Complex Styles</span></div>';
        expect((0, index_1.textToHtml)(message, stylesFormat)).toBe(expected);
    });
    ...
    test("should convert common color names to hex codes", () => {
        ...
        const expected = '<p><span style="color: #800080; background-color: #ffff00;">Colorful Text</span></p>';
        expect((0, index_1.textToHtml)(message, stylesFormat)).toBe(expected);
    });
    ...
    test("should use provided hex color codes directly", () => {
        ...
        const expected = '<p><span style="color: #ABCDEF; background-color: #123456;">Direct Hex Colors</span></p>';
        expect((0, index_1.textToHtml)(message, stylesFormat)).toBe(expected);
    });
    ...
    test("should ignore non-string style values", () => {
        ...
        const stylesFormat = {
            color: "blue",
            invalidProp: 123, // Cast to any to simulate invalid input
            anotherInvalid: null,
        };
        const expected = '<p><span style="color: #0000ff;">Ignoring invalid styles</span></p>';
        expect((0, index_1.textToHtml)(message, stylesFormat)).toBe(expected);
    });
});
